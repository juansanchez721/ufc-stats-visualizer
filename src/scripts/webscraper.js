...
async function getStats(name='conor-mcgregor') {
    ...
    const secondreach = $("c-hero__headline-suffix tz-change-inner")
    const record = $("div.c-hero__header")

    console.log(record[0].children[5].children[0].data.split("•")[1])
    
    
    
    let reach = firstreach[0] ? firstreach[0].children : secondreach[0].children
    let nicknameText = ""
